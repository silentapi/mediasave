...
## Milestone 3 — Gif pipeline

- **Commands** are exactly the spec's two-pass palette invocation (`palettegen=stats_mode=diff`, `paletteuse=dither=bayer:bayer_scale=5:diff_mode=rectangle`), with `-hide_banner -loglevel error` added; `GIF_FPS`/`GIF_MAX_WIDTH` from env (15 / 480). A unit test pins the filter strings.
- **Cache.** `/data/converted/<sha1(mp4 url)>.gif`, reused when < 24 h old and non-empty. Work happens in a per-job temp dir under `/data/tmp/gif-*`, then `os.replace` into place so a half-written gif is never served.
- **Limits.** Source mp4 capped at 50 MB (streamed, aborted early), each ffmpeg pass 60 s (`timeout` error), upstream 4xx → `extract_failed`. Missing ffmpeg → `extract_failed` with a clear message; `/v1/health` reports `ffmpeg: false`.
- **Headers.** The gif source download reuses the item's upstream headers (Referer etc.), relevant if a future provider needs them; Twitter needs none.
- **Prune.** `prune_once` deletes converted gifs > 24 h and temp entries > 1 h; `prune_loop` runs it hourly as a lifespan task. Unit-tested with back-dated mtimes.
- **Fixture.** `tests/fixtures/sample.mp4` is a 3 KB silent 96×64 `testsrc` clip generated by ffmpeg; the real-conversion tests skip when ffmpeg is absent (CI without ffmpeg still passes the rest).
- **Verified live (milestone 2 smoke):** gif tweet → `gif + mp4` items, `/v1/dl` serves the gif from disk with `image/gif` + attachment name, 44 frames at 15 fps (animates), cache hit on second resolve.

## Milestone 4 — TikTok provider

- **Direct page fetch instead of yt-dlp as the primary path.** From a datacenter IP (the build sandbox), a plain `httpx` GET of the post page with a browser User-Agent returns the full `__UNIVERSAL_DATA_FOR_REHYDRATION__` blob for both videos and photo posts, with no WAF challenge and no TLS impersonation. yt-dlp's extractor needs `curl_cffi` impersonation and cannot read photo posts at all, so it is the **fallback** (used only when the page has no rehydration data or shows the challenge page; video posts only). `curl_cffi` stays in `requirements.txt` for that fallback.
- **Two page variants.** Desktop UA → `webapp.video-detail` (has `video.bitrateInfo`, needed to pick a codec); mobile UA → `webapp.reflow.video.detail`, the only variant that includes `imagePost` for photo posts (desktop pages of photo posts carry no detail blob at all). The provider fetches desktop-first for `/video/` URLs and mobile-first for `/photo/`, and tries the other variant if the first has no detail.
- **Video CDN URLs are session-bound.** Tested: the `playAddr`/`bitrateInfo` URLs return `403` unless the request carries (a) the `tt_chain_token` cookie set by the page response, (b) `Referer: https://www.tiktok.com/`, and (c) the same User-Agent as the page fetch. `ttwid`/`tt_csrf_token` are not needed. The dl token therefore carries exactly `Referer`, `User-Agent`, and `Cookie: tt_chain_token=…` (URLs also expire, `x-expires`, well beyond the 15-minute token TTL). In the yt-dlp fallback, the wrapper exports the extractor's cookie jar for `tiktok.com` (`__cookies`) so the same header can be built.
- **Format choice ("best watermark-free mp4").** From `video.bitrateInfo[]`: prefer `CodecType` h264, then highest `Bitrate`; `bytevc2`/h266 skipped (unplayable); h265 only if no h264; `playAddr` as last resort. `downloadAddr` (watermarked) is never used. `bytes` = `PlayAddr.DataSize`, dimensions from `PlayAddr.Width/Height` (all arrive as strings → coerced). Live sample: 576×1024, 5,076,682 bytes, downloads in full through the proxy.
- **Photo mode.** `imagePost.images[]` → one `photo` per image, `imageURL.urlList[0]` (`~tplv-photomode-image.jpeg`, per-image `imageWidth/Height`). The photo CDN needs no headers or cookies (signed URL). Filenames `tiktok_<uniqueId>_<id>_<n>.jpeg`; videos `tiktok_<uniqueId>_<id>.mp4`. Audio-only slideshows (`video.duration == 0`, no `imagePost`) → `no_media`.
- **Status mapping** (from yt-dlp's extractor): `10216`/`10222` → `private`; `10204` (IP blocked) → `private` with a `COOKIES_PATH` hint; redirect to `/login` → `private`; any other non-zero or missing item → `not_found`.
...
### 8.2 yt-dlp TikTok — VERIFIED from a datacenter IP (see Milestone 4 for the outcome: direct page fetch works; yt-dlp is the fallback)
